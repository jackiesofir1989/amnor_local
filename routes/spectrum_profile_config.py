...
router = InferringRouter(tags=['Spectrum Profiles'])


@cbv(router)
class SpectrumProfilesCRUD:
    ROOT_FOLDER = Path('config/spectrum_profiles.yaml')

    @router.get("/spectrum_profiles/")
    def get_file(self):
        """Retrieves a file given a path - if the file is empty is does not return a download link"""
        return FileResponse(self.ROOT_FOLDER, filename='spectrum_profiles.yaml')

    @router.put("/spectrum_profiles/")
